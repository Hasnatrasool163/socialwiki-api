...
// Dynamically converts ANY numeric HTML entity (&#39;, &#41;, etc) safely
function processEntities(rawStr) {
    ...
    const updatedElements = elements.map(el => {
        // Matches &# followed by digits and a semicolon globally
        let fixed = el.replace(/&#(\d+);/g, (match, decCode) => {
...
async function runGlobalCleanup(mode = 'dump') {
    ...
    // Dynamic global query matching any numeric HTML entity pattern
    const query = { address: /&#\d+;/ };

    // =====================================================
    // MODE 1: DUMP PREVIEW (EXCEL-READY CSV)
    // =====================================================
    if (mode === 'dump') {
        const totalMatching = await Address.countDocuments(query);
        console.log(`🔍 Scanning ${totalMatching.toLocaleString()} total entity records for preview generation...`);
        
        const cursor = Address.find(query).cursor();

        const csvWriter = createObjectCsvWriter({
            path: 'entities_clean_preview.csv',
            header: [
        ...
        const duplicateCsvWriter = createObjectCsvWriter({
            path: 'entities_duplicates_preview.csv',
            header: [
                { id: 'postcode', title: 'postcode' },
                { id: 'original_address', title: 'original_address' },
                { id: 'corrected_address', title: 'corrected_address' },
        ...
        for await (const doc of cursor) {
            const { updatedStr, isChanged } = processEntities(doc.address);
        ...
        console.log(`\n✅ SCAN COMPLETE`);
        console.log(`📂 Preview spreadsheet saved: entities_clean_preview.csv`);
        if (duplicateRecords.length > 0) {
            console.log(`⚠️ Alert: ${duplicateRecords.length} unique index conflicts identified! Logged to: entities_duplicates_preview.csv`);
        } else {
    ...
    // =====================================================
    if (mode === 'apply') {
        console.log(`⚠️ Preparation: Reviewing live preview translations...`);
        const cursor = Address.find(query).limit(5);
        
        console.log("\n--- LIVE SNEAK PEEK ---");
        for await (const doc of cursor) {
            const { updatedStr } = processEntities(doc.address);
        ...
        const confirm = await new Promise(res => {
            rl.question(`Proceed to execute cleanup on all dynamic HTML entities and clear index conflicts? (Y/N): `, ans => {
        ...
        console.log(`🚀 Executing high-speed dynamic bulk writes...`);
        ...
        console.log(`\n✅ COMPLETE! Successfully processed ${updateCount} rows and cleared ${deleteCount} unique conflicts.`);
    }

...
async function runCorrection(mode = 'dump') {
    ...
    // ----------------------------------------------------
    // MODE 1: DUMP ONLY (Outputs ultra-clean CSV files for Excel)
    // ----------------------------------------------------
    if (mode === 'dump') {
        ...
        // CHANGED: Replaced text writeStream with a structured CSV Writer for Excel optimization
        const addressPreviewCsvWriter = createObjectCsvWriter({
            path: 'address_preview.csv',
            header: [
        ...
        const addressPreviewRecordsCsv = [];
        ...
        for (const item of candidateRecords) {
            ...
            // Push to the regular preview list
            addressPreviewRecordsCsv.push({
        ...
        // Write both CSV outputs cleanly
        await addressPreviewCsvWriter.writeRecords(addressPreviewRecordsCsv);

        if (duplicateRecordsCsv.length > 0) {
            await duplicateCsvWriter.writeRecords(duplicateRecordsCsv);
            ...
            await duplicateCsvWriter.writeRecords([]); // Clear out file if empty
            console.log(`✅ Clean scan: Zero unique index conflicts or duplicate deletes detected.`);
        }

        console.log(`✅ Excel-ready CSV preview generated: address_preview.csv`);
